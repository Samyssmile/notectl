...
### Class-Based HTML Export

To produce CSP-compliant exported HTML, use `cssMode: 'classes'`:

```ts
const { html, css } = editor.getContentHTML({ cssMode: 'classes' });
```

This replaces all inline `style` attributes with CSS class names and returns the collected stylesheet alongside the HTML:

```html
<!-- html -->
<p class="notectl-align-center">
  Normal text and
  <span class="notectl-s0">styled text</span>.
</p>
```

```css
/* css */
.notectl-s0 { color: red; font-size: 18px; }
.notectl-align-center { text-align: center; }
```

The generated CSS contains **only** the rules used in the document. Identical style combinations are deduplicated into a single class.

#### Rendering on a CSP-Strict Page

Inject the generated CSS into a `<style>` element with a nonce, or serve it as an external stylesheet:

```ts
// Option 1: Nonce-based <style> element
const { html, css } = editor.getContentHTML({ cssMode: 'classes' });

const style = document.createElement('style');
style.nonce = window.__CSP_NONCE__;
...
```ts
// Option 2: Store CSS alongside HTML and serve as external stylesheet
...
|----------|-----------------|
| Storing HTML for email or external apps | `cssMode: 'inline'` (default) — self-contained |
| Displaying HTML on a CSP-strict page | `cssMode: 'classes'` — no inline styles |
| Clipboard copy/paste | Automatic — always uses inline styles for compatibility |
