...
notectl is the first rich text editor that works under strict Content Security Policy headers **without requiring `'unsafe-inline'` for styles**. This is the default — zero configuration needed for modern browsers.
