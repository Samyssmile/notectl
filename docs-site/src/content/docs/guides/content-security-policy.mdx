...
Use the built-in `injectContentStyles` utility for CSP-compliant CSS injection:
...
// Inject with nonce for strict CSP
...
Or serve as an external stylesheet:
...
## `injectContentStyles` API Reference
