...
describe('ImageCommands', () => {
	...
	describe('setImageAlign', () => {
		it('sets alignment to left', async () => {
			const state = stateBuilder()
				.paragraph('', 'b1')
				.block('image', '', 'img1', { attrs: { src: 'test.png', alt: '', align: 'center' } })
				.nodeSelection('img1')
			...
			const result = pm.executeCommand('setImageAlignLeft');

			expect(result).toBe(true);
			expect(dispatch).toHaveBeenCalled();
			const imgBlock = getState().doc.children[1];
			expect(imgBlock?.attrs?.align).toBe('left');
		});

		it('sets alignment to right', async () => {
			const state = stateBuilder()
				.paragraph('', 'b1')
				.block('image', '', 'img1', { attrs: { src: 'test.png', alt: '', align: 'center' } })
				.nodeSelection('img1')
			...
			pm.executeCommand('setImageAlignRight');

			const imgBlock = getState().doc.children[1];
			expect(imgBlock?.attrs?.align).toBe('right');
		});
