...
describe('ImageNodeView', () => {
	...
	describe('alignment toolbar', () => {
		it('selectNode creates alignment toolbar', () => {
			const uploadStates = new Map<BlockId, UploadState>();
			const factory = createImageNodeViewFactory(DEFAULT_IMAGE_CONFIG, uploadStates);
			const node = makeImageBlock();
			const view = factory(node, makeState, vi.fn());
			...
			view.selectNode?.();
			const toolbar = view.dom.querySelector('.notectl-image__align-toolbar');
			expect(toolbar).not.toBeNull();
		});

		it('alignment toolbar has 3 buttons', () => {
			const uploadStates = new Map<BlockId, UploadState>();
			const factory = createImageNodeViewFactory(DEFAULT_IMAGE_CONFIG, uploadStates);
			const node = makeImageBlock();
			const view = factory(node, makeState, vi.fn());
			...
			view.selectNode?.();
			const buttons = view.dom.querySelectorAll('.notectl-image__align-btn');
		...
		it('active alignment button is highlighted', () => {
			const uploadStates = new Map<BlockId, UploadState>();
			const factory = createImageNodeViewFactory(DEFAULT_IMAGE_CONFIG, uploadStates);
			const node = makeImageBlock({ align: 'left' });
			const state = makeState({ align: 'left' });
			const view = factory(node, () => state, vi.fn());

			view.selectNode?.();
			const buttons = view.dom.querySelectorAll('.notectl-image__align-btn');
			const activeButtons: Element[] = [];
			for (const btn of buttons) {
				if (btn.classList.contains('notectl-image__align-btn--active')) {
					activeButtons.push(btn);
				}
			}
			expect(activeButtons).toHaveLength(1);
			expect(activeButtons[0]?.getAttribute('aria-label')).toBe('Align left');
		});

		it('center alignment is active by default', () => {
			const uploadStates = new Map<BlockId, UploadState>();
			const factory = createImageNodeViewFactory(DEFAULT_IMAGE_CONFIG, uploadStates);
			const node = makeImageBlock();
			const view = factory(node, makeState, vi.fn());
			...
			expect(activeButtons).toHaveLength(1);
			expect(activeButtons[0]?.getAttribute('aria-label')).toBe('Align center');
		});

		it('deselectNode removes alignment toolbar', () => {
			const uploadStates = new Map<BlockId, UploadState>();
			const factory = createImageNodeViewFactory(DEFAULT_IMAGE_CONFIG, uploadStates);
			const node = makeImageBlock();
			const view = factory(node, makeState, vi.fn());
		...
		it('destroy removes alignment toolbar', () => {
			const uploadStates = new Map<BlockId, UploadState>();
			const factory = createImageNodeViewFactory(DEFAULT_IMAGE_CONFIG, uploadStates);
			const node = makeImageBlock();
			const view = factory(node, makeState, vi.fn());
